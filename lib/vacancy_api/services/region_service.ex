defmodule VacancyApi.RegionService do
  @moduledoc """
  Gets Latitude and Longitude and return region which this coordinates belongs
  """

  @pacific_ocean_name RegionEnum.__enum_map__() |> Keyword.keys() |> Enum.reverse() |> hd()
  ...
  """
  @spec which_region(float, float, list) ::
          {:ok, atom} | {:error, :invalid_params} | {:error, :unexpected_error}
  def which_region(latitude, longitude, regions_polygons)
    ...
    regions_polygons
    |> Enum.find(fn {_region, polygons} ->
      Enum.any?(polygons, &Topo.intersects?(&1, map_point))
    end)
    |> case do
      {region, _polygons} -> {:ok, region}
      nil -> {:ok, @pacific_ocean_name}
    end
  end

  def which_region(_, _, _), do: {:error, :invalid_params}

  @doc """
  Return Keyword of polygons those describe zones related to according regions. Those looks like:
  ...
  def get_polygons do
    [
      ...
      antarctic_name,
      _pacific_ocean
    ] = Keyword.keys(RegionEnum.__enum_map__())
